...
enum class RedUrlVideoLiteEngine{
    CHAINTECH,
    ANDROID,
    COMPOSE
}
...
fun RedUrlVideoLite(url: String, engine: RedUrlVideoLiteEngine = RedUrlVideoLiteEngine.ANDROID) {
    when (engine) { // when более идиоматичен для такого переключения
        RedUrlVideoLiteEngine.ANDROID -> RedUrlVideoLiteAndroid(url = url)
        RedUrlVideoLiteEngine.COMPOSE -> RedUrlVideoLiteCompose(url = url)
        RedUrlVideoLiteEngine.CHAINTECH -> RedUrlVideoLiteChaintech(url = url)
    }
}

@OptIn(UnstableApi::class)
@Composable
fun RedUrlVideoLiteCompose(url: String) {
...
@OptIn(UnstableApi::class)
@Composable
fun RedUrlVideoLiteChaintech(url: String) {
