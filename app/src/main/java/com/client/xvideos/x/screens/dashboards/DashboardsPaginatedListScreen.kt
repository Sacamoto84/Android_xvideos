...
fun DashboardsPaginatedListScreen(pageIndex: Int, vm: ScreenXDashBoardsScreenModel) {
    ...
    // Список берём из кэша VM (переживает переключение вкладок), а не из локального remember.
    val l = remember(pageIndex) { vm.pageFor(pageIndex) }

    LaunchedEffect(key1 = pageIndex, key2 = CountryState.updateTrigger) {
        // При смене страны кэш всех страниц инвалидируем.
        if (vm.cacheCountryTrigger != CountryState.updateTrigger) {
            vm.cacheCountryTrigger = CountryState.updateTrigger
            vm.clearPageCacheContents()
        }
        // Грузим из сети только если для страницы ещё нет данных — иначе возврат
        // на дашборды переиспользует кэш без повторной загрузки.
        if (l.isEmpty()) {
    ...
    // Состояние прокрутки берём из VM, чтобы позиция не сбрасывалась при переключении вкладок.
    val gridState = remember(pageIndex) { vm.gridStateFor(pageIndex) }
